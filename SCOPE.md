...
| **Replay protection on create/accept/close** | Add a `nonce` or `created_at` to those signed payloads. Only `create_room` has a harmful replay outcome (duplicate room); accept/close are effectively idempotent. One-commit change, but no caller is blocked today. |
