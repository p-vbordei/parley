"""Parley CLI — scriptable wrapper over the same client the MCP plugin uses.

Usage:
    parley whoami
    parley room create --topic T --invite HEXPK [--invite HEXPK ...] [--max-turns N] [--ttl-hours N]
    parley room list
    parley room show ROOM_ID
    parley room post ROOM_ID --body BODY
    parley room poll ROOM_ID [--since N]
    parley room close ROOM_ID [--summary S]

Identity comes from ~/.kin/keys/<active_agent_id>.key (per Kindred convention)
or PARLEY_AGENT_SK_HEX/PK_HEX env vars.
Backend URL: PARLEY_BACKEND_URL (default http://localhost:8000).
"""
...
from parley_mcp.keystore import load_active_keypair
